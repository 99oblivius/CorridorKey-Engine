"""Unit tests for CorridorKeyModule.engine_factory — no GPU/MLX required."""
...
from CorridorKeyModule.engine_factory import (
    BACKEND_ENV_VAR,
    MLX_EXT,
    TORCH_EXT,
...
class TestResolveBackend:
    ...
    def test_explicit_mlx_on_non_apple_raises(self):
        with mock.patch("CorridorKeyModule.engine_factory.sys") as mock_sys:
    ...
    def test_auto_darwin_no_mlx_package(self):
        with (
            mock.patch("CorridorKeyModule.engine_factory.sys") as mock_sys,
            mock.patch("CorridorKeyModule.engine_factory.platform") as mock_platform,
        ):
...
class TestDiscoverCheckpoint:
    def test_exactly_one(self, tmp_path):
        ...
        ckpt.touch()
        with mock.patch("CorridorKeyModule.engine_factory.CHECKPOINT_DIR", str(tmp_path)):
            result = _discover_checkpoint(TORCH_EXT)
            assert result == ckpt

    def test_zero_raises(self, tmp_path):
        with (
            mock.patch("CorridorKeyModule.engine_factory.CHECKPOINT_DIR", str(tmp_path)),
