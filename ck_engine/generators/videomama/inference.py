...
def run_inference(
    ...
    logger.info(f"Processing {len(frames_resized)} frames in chunks of {chunk_size}...")
    ...
    for i in range(0, len(frames_resized), chunk_size):
        ...
        logger.info(f"  Running inference on chunk {i//chunk_size + 1}/{len(frames_resized)//chunk_size + 1} ({len(chunk_frames)} frames)...")
