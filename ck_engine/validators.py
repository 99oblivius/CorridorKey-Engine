...
def ensure_output_dirs(clip_root: str) -> dict[str, str]:
    ...
    """
    out_root = os.path.join(clip_root, Dir.OUTPUT)
    dirs = {
        "root": out_root,
        "fg": os.path.join(out_root, Dir.FG),
        "matte": os.path.join(out_root, Dir.MATTE),
        "comp": os.path.join(out_root, Dir.COMP),
        "processed": os.path.join(out_root, Dir.PROCESSED),
    }
