...
from .config import DEFAULT_IMG_SIZE, get_exr_write_flags
from .frame_io import read_image_frame, read_mask_frame
...
# Enable OpenEXR support in OpenCV once at import time. This is a
# process-global flag; setting it repeatedly from concurrent thread-pool
# workers (as was done previously) is redundant and technically a data race.
...
class PipelineConfig:
    ...
    cpus: int = 0  # 0 = os.cpu_count(); minimum 4
    devices: list[str] | None = None  # None = auto-detect
    backend: str | None = None  # "torch", "mlx", or None for auto
    optimization_config: OptimizationConfig | None = None  # None uses engine defaults
    comp_format: str = "exr"  # "exr", "png", or "none"
    gpu_resilience: bool = False  # True = requeue OOM frames, False = halt pipeline

    _MIN_CPUS: ClassVar[int] = 4

    def __post_init__(self) -> None:
        if self.cpus and self.cpus < self._MIN_CPUS:
            logger.warning("--cpus %d below minimum (%d), clamping", self.cpus, self._MIN_CPUS)
            self.cpus = self._MIN_CPUS


# Re-use the canonical definition from pipeline to avoid drift.
from .pipeline import InferenceSettings  # noqa: E402,TC001
...
def _read_frame_pair(
    ...
    """
    img = read_image_frame(input_path)
    if img is None:
        logger.warning("Failed to read input frame: %s", input_path)
        return None

    mask = read_mask_frame(alpha_path, frame_index=index)
    if mask is None:
...
# Sentinel color_conversion values for PNG output (float32 → uint8).
_PNG_PREP = -1       # RGB float32 → BGR uint8
_PNG_PREP_RGBA = -2  # RGBA float32 → BGRA uint8
...
def _write_frame_outputs(
    ...
    cv2.convertScaleAbs, and cv2.imwrite are C code that releases
    the GIL, so writer threads truly parallelize.
...
class AsyncInferencePipeline:
    ...
    def load_engines(self) -> None:
        ...
        if self.config.cpus:
            torch.set_num_threads(self.config.cpus)
            logger.info("PyTorch CPU threads limited to %d", self.config.cpus)
    ...
    def process_clip(
        ...
        if self.config.cpus:
            cores = min(cores, self.config.cpus)
        ...
        gpus_alive = [num_gpus]  # protected by failed_lock (decrement-and-check must be atomic)
        ...
        write_futures: set[tuple[int, Any]] = set()
        ...
        def _resolve_and_submit(item: tuple[FramePacket, Any], write_pool: ThreadPoolExecutor) -> None:
            ...
            _exr_flags = get_exr_write_flags()
            write_list: list[tuple[np.ndarray, str, list[int] | None, int | None]] = [
            ...
            if self.config.comp_format != "none" and comp is not None:
                is_rgba = comp.ndim == 3 and comp.shape[2] == 4
                if self.config.comp_format == "png":
                    png_prep = _PNG_PREP_RGBA if is_rgba else _PNG_PREP
                    write_list.append((comp, os.path.join(output_dirs["comp"], f"{stem}.png"), None, png_prep))
                else:
                    color_conv = cv2.COLOR_RGBA2BGRA if is_rgba else cv2.COLOR_RGB2BGR
                    write_list.append((comp, os.path.join(output_dirs["comp"], f"{stem}.exr"), _exr_flags, color_conv))
        ...
        def inference_worker(device_str: str, engine: Any, write_pool: ThreadPoolExecutor) -> None:
            ...
            dynamo_retried: set[int] = set()  # frame indices already retried for dynamo/FX errors
            ...
            while not self._shutdown_event.is_set():
                ...
                try:
                    ...
                    logger.error(
                        "\033[1mCUDA OOM on %s for frame %d\033[0m",
                        device_str,
                        packet.index,
                    )
                    ...
                    if self.config.gpu_resilience:
                        with failed_lock:
                            gpus_alive[0] -= 1
                            remaining = gpus_alive[0]
                        if remaining > 0:
                            logger.info(
                                "Requeuing frame %d for another GPU (%d remaining)",
                                packet.index,
                                remaining,
                            )
                            work_q.put(packet)
                        else:
                            logger.error("All GPUs offline — halting pipeline")
                            with failed_lock:
                                failed_frames.append(packet.index)
                            self._shutdown_event.set()
                    else:
                        # Default: halt the entire pipeline
                        logger.error(
                            "Halting pipeline — use --gpu-resilience to continue on remaining GPUs"
                        )
                        self._shutdown_event.set()
                        with failed_lock:
                            failed_frames.append(packet.index)
                    break  # This GPU is offline either way
                except Exception as e:
                    err_msg = str(e)
                    if "symbolically trace" in err_msg and packet.index not in dynamo_retried:
                        logger.warning(
                            "Dynamo/FX conflict on frame %d — resetting dynamo and retrying once",
                            packet.index,
                        )
                        torch._dynamo.reset()
        ...
        try:
            ...
            # Drain any frames left in work_q (e.g. requeued after all GPUs died)
            orphaned = 0
            while not work_q.empty():
                item = work_q.get_nowait()
                if item is not _SHUTDOWN and isinstance(item, FramePacket):
                    with failed_lock:
                        failed_frames.append(item.index)
                    orphaned += 1
            if orphaned:
                logger.error("%d frame(s) lost — no GPUs available to process them", orphaned)
