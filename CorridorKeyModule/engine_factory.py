...
def _wrap_mlx_output(raw: dict, despill_strength: float, auto_despeckle: bool, despeckle_size: int) -> dict:
    ...
    if auto_despeckle:
        processed_alpha = cu.clean_matte(alpha, area_threshold=despeckle_size, dilation=DEFAULT_MATTE_DILATION, blur_size=DEFAULT_MATTE_BLUR)
    else:
...
DEFAULT_MLX_TILE_SIZE = DEFAULT_TILE_SIZE
...
) -> _BaseCorridorKeyEngine | _MLXEngineAdapter:
